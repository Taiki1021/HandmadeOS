...
struct FATInfo{
	...
	ushort	version;
	uint	rootdircluster;
	ushort	fsysinfolocation;
	ushort	bootcopylocation;
	uchar	driveno;
	uchar	bootsig;
	uint	volumeid;
...
void readcluster(struct FATInfo* fathead,uint cluster,char* buf,int size){
	...
	else		blockcpy(buf,DISK+start+(cluster-2)*0x200,sizeof(char)*0x200);
...
void tinyls(){
	...
	for(A=0;rootdir[0x40*A]!=0 && rootdir[0x40*A]!=0xe5;A++){
		blockcpy(name,&rootdir[0x40*A+0x20],sizeof(char)*11);
